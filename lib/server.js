...
/* helpers.sendTwilioSMS(
  '206257213',
  'I just sent a message via node.js',
  (err) => {
...
server.unifiedServer = (req, res) => {
  ...
  req.on('end', () => {
    ...
    choosendHandler(data, (statusCode, payload) => {
      ...
      const payloadString = JSON.stringify(payload); // this is not the payload we received, the handler sending back to the user
